...
export function NoteNode({ id, data, selected }: NodeProps & { data: NoteNodeData }) {
  ...
  return (
    <div className={`relative flex h-full min-h-[140px] min-w-[220px] flex-col overflow-hidden rounded-xl border bg-[rgb(var(--surface))] shadow-md transition-shadow ${
      selected ? 'border-[rgb(var(--accent))] shadow-[0_0_0_2px_rgba(var(--accent),0.2)]' : 'border-[rgb(var(--border))]'
    }`}>
      <NodeResizer minWidth={220} minHeight={140} isVisible={selected} lineStyle={{ borderColor: 'rgb(var(--accent))' }} handleStyle={{ borderColor: 'rgb(var(--accent))' }} />
