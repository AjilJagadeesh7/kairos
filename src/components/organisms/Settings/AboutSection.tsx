import { BrainCircuit, FolderSync, Network, StickyNote, SquareKanban, ShieldCheck, ExternalLink, CalendarDays, History, GitFork, ClipboardCopy, Trash2, ScrollText } from 'lucide-react'
  ...
  return (
    <SectionCard title="Diagnostics">
      <div className="space-y-3">
        ...
        {expanded && (
          <pre className="max-h-48 overflow-y-auto rounded-lg bg-[rgb(var(--surface-2))] p-3 text-[10px] leading-relaxed text-[rgb(var(--text-3))]">
            {entries.length === 0
              ? '(no entries)'
              : entries.slice(-30).map(e => JSON.stringify(e)).join('\n')}
          </pre>
        )}
      </div>
    </SectionCard>
